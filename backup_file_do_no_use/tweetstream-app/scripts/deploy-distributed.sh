...
if [ -n "$WORKER_NODE" ]; then
    ...
    if sshpass -p "1234" ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no sanzad@$WORKER_NODE "echo '1234' | sudo -S grep -q '$REGISTRY_HOST' /etc/containerd/config.toml" 2>/dev/null; then
